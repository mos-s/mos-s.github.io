...
// No of sample blocks in circular buffer
...
class SamplesBufferWasm {
   ...
   * @param  {number=} maxChannelCount Maximum number of channels.
   */
  constructor(wasmModule, length, channelCount, iSampleBlocks) {//}, maxChannelCount) {
    // The |channelCount| must be greater than 0, and less than or equal to
    // the maximum channel count.
    this._isInitialized = false;
    this._module = wasmModule;
    ...
    //this._maxChannelCount = maxChannelCount
       ...
       // : channelCount;
       this._channelCount = channelCount;
       this._iSampleBlocks = iSampleBlocks;
       this._allocateHeap();
    this._isInitialized = true;
  }
  ...
  _allocateHeap() {
    const channelByteSize = this._length * BYTES_PER_SAMPLE;
    const dataByteSize = this._channelCount * channelByteSize * this._iSampleBlocks;
    this._dataPtr = this._module._malloc(dataByteSize);
    this._channelData = [];
    for (let i = 0; i < this._channelCount; ++i) {
      let startByteOffset = this._dataPtr + i * channelByteSize;
...
export {
  ...
  SAMPLE_BLOCKS,
  HeapAudioBuffer,
  SamplesBufferWasm as SamplesBufferWasm,
  RingBuffer,
};
