...
DESCRIPTION
  Cannibalised from https://github.com/peterkhayes/pitchfinder yin.ts
...
export function yinOld(float32AudioBuffer) {
  ...
  return window.iSampleRate / betterTau;
}
//}
const audioPacketSamples = 256;
//const window.maxSampleWl = 600;
export function yinNew(float32AudioBuffer) {
  ...
  if (iRingPackets == null) {
    iRingPackets = Maths.ceil(window.maxSampleWl / audioPacketSamples);
    let ringPackets = new float32AudioBuffer[iRingPackets]();
    iCurPacket = 0;
  }
  ringPackets[iCurPacket] = float32AudioBuffer;
  iCurPacket = iCurPacket + 1;
  if (iCurPacket >= iRingPackets) {
    iCurPacket = 0;
  }

  if (ringPackets[iRingPackets - 1] == undefined) { 
    return 0; //? for now
  } else { // Compute pitch!
    // Set up the yinBuffer as described in step one of the YIN paper.
    const yinBufferLength = bufferSize / 2;
...
export function yin(float32AudioBuffer, max_sample_wl_param, iStartIndex) {
  // Set buffer size to the highest power of two below the provided buffer's length.
  /*let bufferSize;
  for (bufferSize = 1; bufferSize < float32AudioBuffer.length; bufferSize *= 2);
  bufferSize /= 2;
*/
  // Set up the yinBuffer as described in step one of the YIN paper.
  const yinBufferLength = max_sample_wl_param;//bufferSize / 2;
  const yinBuffer = new Float32Array(yinBufferLength);
